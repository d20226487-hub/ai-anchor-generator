 ...
 *  - Link Type is REQUIRED (parseCsvTextV2 default), matching V2's input model.
 *  - Prompt preview uses actionPreviewPromptV2.
 *  - V2 copy ("anchors requested", V2 inputs description).
 *
 * Pre-fills name / site description / provider / model / CSV from the existing job and
 * offers Save only / Save & resume (partial/paused/cancelled) / Save & rerun — same
...
export function EditJobV2Client({ job, settings }: { job: Job; settings: SettingsBlob }) {
  ...
  // V2 requires Link Type (unlike Пироги) — use the parser's default.
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold">{t("editJob.headingV2", { name: job.name })}</h1>
            ...
            <CardBody>
              {summary.rowCount === 0 ? (
                ...
                <div className="space-y-3">
                  <div>
                    <div className="text-3xl font-semibold tabular-nums text-[var(--color-text)]">{summary.totalAnchors}</div>
